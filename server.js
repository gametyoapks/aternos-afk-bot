...
function startBot(config) {
  ...
  bot.on('end', (reason) => {
    scheduleReconnect(`Bağlantı kesildi${reason ? ': ' + reason : ''}. 5 saniye içinde yeniden bağlanılıyor...`);
  });

  bot.on('kicked', (reason) => {
    let reasonText = '';
    try { reasonText = JSON.parse(reason)?.text || reason; } catch (e) { reasonText = reason; }
    scheduleReconnect(`Sunucudan atıldı: ${reasonText}. 5 saniye içinde yeniden bağlanılıyor...`);
  });
}
