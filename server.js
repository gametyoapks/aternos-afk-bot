...
function scheduleReconnect(slotId, reasonLabel, rawReasonText, isPreSpawnGenericFail) {
  ...
  if (s.consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
    const throttleNote = classification.category === 'THROTTLE'
      ...
      : '';
    broadcastLog(slotId, 'ERROR', `${reasonLabel} ${MAX_CONSECUTIVE_FAILURES} kez art arda başarısız oldu, otomatik olarak durduruldu.${throttleNote} Sunucu IP'sini/durumunu kontrol edip tekrar başlat.`);
    s.reconnectScheduled = false;
    stopBot(slotId);
    return;
  }
...
app.post('/api/start', (req, res) => {
  let { ip, port = 25565, alreadyRegistered = false, slot } = req.body || {};
